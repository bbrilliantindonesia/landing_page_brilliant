// Data untuk Program Bimbel
export const programs = [
  {
    id: 1,
    title: "Program bimbel TKA",
    subtitle: "SD, SMP & SMA",
    description:
      "Bimbingan belajar untuk TKA SD, SMP, dan SMA dengan materi terstruktur, latihan rutin, dan pendampingan intensif agar siap menghadapi ujian sekolah.",
    image: "tka.webp",
  },
  {
    id: 2,
    title: "Program bimbel",
    subtitle: "SNBT & UM PTN",
    description:
      "Program persiapan masuk PTN favorit dengan fokus latihan soal SNBT dan UM, strategi pengerjaan, serta pembahasan mendalam bersama mentor berpengalaman.",
    image: "snbt.webp",
  },
  {
    id: 3,
    title: "Program bimbel",
    subtitle: "Tes CPNS & CPPPK",
    description:
      "Persiapan tes CPNS dan PPPK mulai dari TWK, TIU, hingga TKP dengan simulasi CAT dan pembahasan soal yang sesuai standar terbaru.",
    image: "cpns.webp",
  },
  {
    id: 4,
    title: "Program bimbel",
    subtitle: "Tes Polisi & TNI",
    description:
      "Program khusus persiapan seleksi Polisi dan TNI meliputi akademik, psikotes, dan pembinaan mental agar lebih siap dan percaya diri saat tes.",
    image: "polisi_tni.webp",
  },
  {
    id: 5,
    title: "Program bimbel",
    subtitle: "Tes Kedinasan",
    description:
      "Bimbingan intensif untuk tes sekolah kedinasan dengan materi terarah, latihan soal berkala, dan strategi lolos seleksi tahap demi tahap.",
    image: "kedinasan.webp",
  },
  {
    id: 6,
    title: "Program bimbel",
    subtitle: "Tes BUMN & BUMS",
    description:
      "Persiapan tes rekrutmen BUMN dan BUMS dengan fokus kemampuan dasar, psikotes, dan simulasi soal sesuai kebutuhan dunia kerja.",
    image: "bumn.webp",
  },
];


// Data untuk Testimonials
export const testimonials = [
  {
    id: 1,
    name: "Chandra Rafiyanto",
    university: "Lulus CPT Kemenkumham",
    image: "/photo/testimonial/Chandra.webp",
    quote:
      "Brilliant Indonesia bener-bener game changer 🔥 Materinya relevan, pembahasannya ngena, dan cara ngajarnya gak ngebosenin sama sekali. Dari yang awalnya bingung, jadi paham dan lebih pede buat berkembang. Highly recommended buat kamu yang mau level up, serius.",
  },
  {
    id: 2,
    name: "Rendra Ardiansyah",
    university: "Lulus CPNS Tenaga Kesehatan",
    image: "/photo/testimonial/Rendra.webp",
    quote:
      "Mentor-mentornya sangat berpengalaman dan sabar dalam mengajar. Materi yang diberikan lengkap dan up-to-date sesuai kurikulum terbaru.",
  },
  {
    id: 3,
    name: "Kenzie Muhammad Rizieq",
    university: "Lulus SNBT ke Universitas Gajah Mada",
    image: "/photo/testimonial/Kenzie.webp",
    quote:
      "Program garansi tahun depan membuat saya lebih tenang dalam belajar. Alhamdulillah lolos di percobaan pertama!",
  },
  {
    id: 4,
    name: "Muhammad Hafidz",
    university: "Lulus CPNS Dosen",
    image: "/photo/testimonial/Hafizh.webp",
    quote:
      "Bimbel terbaik yang pernah saya ikuti! Sistem belajarnya fleksibel, bisa online maupun offline sesuai kebutuhan.",
  },
  {
    id: 5,
    name: "Nata Akbari Syaifullah",
    university: "Lulus CPNS Kemenkumham",
    image: "/photo/testimonial/Nata.webp",
    quote:
      "Lolos CPNS berkat bimbingan dari Brilliant Indonesia. Sangat membantu untuk persiapan ujian sesungguhnya.",
  },
  {
    id: 6,
    name: "Gifari Rasyid",
    university: "Lulus TNI AD",
    image: "/photo/testimonial/Gifari.webp",
    quote:
      "Dari awal tidak percaya diri, sekarang sudah diterima di TNI AD. Terima kasih Brilliant Indonesia!",
  },
];

// Data untuk Expandable Testimonials Grid
export const expandableTestimonials = [
  {
    id: 1,
    name: "Azkya Mursalim",
    acceptance: "Lulus CPNS Kejaksaan",
    image: "/photo/expandTestimonial/AzkyaMursalim_CPNSJaksa.jpg",
    quote:
      "Materi SKD & SKB-nya daging semua! Mentornya paham betul pola soal CPNS Kejaksaan tahun ini. Alhamdulillah strategi 'tembak jitu' yang diajarin bikin saya lolos perankingan di formasi Jaksa.",
  },
  {
    id: 2,
    name: "Geri",
    acceptance: "Lulus Bintara Polri",
    image: "/photo/expandTestimonial/Geri_Polisi.jpg",
    quote:
      "Persiapan tes akademik dan psikotes di sini bener-bener terarah. Latihan soalnya akurat banget sama tes aslinya, jadi pas hari H udah nggak kaget lagi. Bangga bisa pakai seragam ini!",
  },
  {
    id: 3,
    name: "Raju & Yusran",
    acceptance: "Lulus Bintara Polri",
    image: "/photo/expandTestimonial/Raju&Yusran_Polisi.jpg",
    quote:
      "Belajar bareng di Brilliant bikin kita berdua makin semangat. Tips trik ngerjain soal matematikanya cepet banget, ngebantu banget pas tes akademik Polri kemarin. Sukses bareng itu nyata!",
  },
  {
    id: 4,
    name: "Yuriga",
    acceptance: "Lulus CPNS Kemenkumham",
    image: "/photo/expandTestimonial/Yuriga _CPNSKemenkumham.jpg",
    quote:
      "Awalnya ragu bisa tembus Kemenkumham karena saingannya ribuan. Tapi berkat bimbingan intensif dan tryout rutin di sini, skor SKD saya melonjak drastis dan aman sampai tahap akhir.",
  },
  {
    id: 5,
    name: "Zaki",
    acceptance: "Lulus STIP Jakarta",
    image: "/photo/expandTestimonial/Zaki_STIP.jpg",
    quote:
      "Masuk sekolah kedinasan impian kayak STIP rasanya mustahil kalau belajar sendiri. Brilliant ngebantu banget bedah soal TPA dan TBI-nya sampai ke akar-akarnya. Recommended buat pejuang kedinasan!",
  },
];
